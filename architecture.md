...
### 2026-07-19 — Session 24: fixed GA4 dev-traffic pollution, data-fill pass (112->123 published), niche #2 research (rejected)
- **GA4 review found a real bug, not real traffic.** Human shared GA4 screenshots showing 44 "active users" over 7 days across 7 countries — looked exciting but every tell said otherwise: Direct-dominant with zero Organic Search, multi-country spread for a hyper-local US directory 5 days post-launch, and specific listing-page hits matching manual testing patterns. Root cause: the gtag snippet had no hostname check, so every local dev-server preview and Playwright verification screenshot this project's own sessions ran fired real GA4 events. Fixed: gtag now only initializes when `location.hostname` is the real production domain. Real traffic signal is still zero, which is correct/expected at day 5.
- **Niche #2 research (human asked to explore, then said hold off starting a new workspace until fully validated)**: ran real SERP + competitor checks (WebSearch, subagents hit a session limit and failed, did the research directly instead) across 10 candidates. Golf cart/ATV/scaffolding/e-bike rental: weak (existing cost-guide content sites or thin TAM). Party bus, pontoon/boat, storage containers, costume rental: REJECT (real funded marketplaces or national brands already own each — Price4Limo/PartyBus360, GetMyBoat/Boatsetter/sail.me, PODS/U-Haul, Men's Wearhouse/Generation Tux). **Generator rental looked like the best candidate on keyword data alone** (human pulled real Ahrefs data: `generator rental near me` >1000 vol Easy KD) but the deep validation killed it: Sunbelt/United Rentals/Home Depot publish real per-unit prices online already (the exact gap our moat needs doesn't exist), and RNTR (38,000 listings, 40+ states, launched Nov 2024) is a live structural clone of our whole model. Same discipline that killed pickleball in session 2. Proposed next candidates (mobile bartending, petting zoo/pony parties) not yet validated — human deferred this thread to focus on filling the current site's data instead.
- **Data-fill pass on eventrentalcosts.com** (human's priority): Apify credit had refreshed on its monthly cycle ($0.32/$5 used, not exhausted as assumed). Two-pronged:
  - **Free win: re-extracted pricing for the 10 fee-only WARN listings** (flagged session 23) now that the real Firecrawl key works — real per-item prices recovered for Dow Oak Photo Booth ($400 photo booth) and Party To Go ($300-400 bounce houses); 5 stayed genuinely fee-only (no real item pricing exists in their scraped content); 1 (Go Jump and Slide Inflatables) had nothing at all and correctly dropped from published. Also swept the global ai_extract queue (leftover scraped-but-unprocessed businesses from session 22/23's rate-limit fallout) — real wins: LIT Charleston (98 pricing items!), A Tailor Made Event (81), A & A Rentals & Sales Inc (27), Creative Event Rentals (11), Sage Hill Rentals (41), plus several smaller ones.
  - **Apify spend: found real untapped supply** — Columbus and Knoxville each had 4 suburb cities in `metros.py` with ZERO raw data ever pulled (never ingested, not a "ceiling," just never tried); Charleston had 2. Ingested Knoxville's 4 (Farragut/Maryville/Oak Ridge/Sevierville — mostly matched back to Knoxville-proper or off-category, only 2 real new candidates) and Columbus's 4 (Dublin/Westerville/Grove City/Hilliard — 5 real new candidates). Richmond and Greenville were re-checked and confirmed to already be at their real processing ceiling (all suburbs already ingested in prior sessions, zero leftover unscraped candidates) — no wasted spend there.
- **Result: 112 -> 123 published listings.** Charleston 10->16, Knoxville 5->8, Columbus 7->8. All other metros unchanged. No metro crossed a new threshold this session (Greenville stays at 5, Pittsburgh at 1 — genuine ceiling, not re-tested this round since no new data was added there).
- Cleaned up temp files (`knoxville_runs.json`, `columbus_runs.json` — scratch run-ID tracking, not meant to be committed).
- **CORRECTION, same day: the 123 count above was itself sitting on undeleted hallucinated data.** Human spotted "10x10 tents from $0" live on the Knoxville metro page — a direct violation of the real-data-only promise. Root cause traced to `validate_pricing.py`: the print loop that reports removed rows ran BEFORE the actual `DELETE`, and that print loop crashes on Windows' default console codepage (cp1252) whenever a snippet contains a character outside it (e.g. U+2033, the inches double-prime). Every time it crashed, the script correctly *identified* hallucinated rows but never reached the `DELETE` — meaning bad rows had been accumulating silently across an unknown number of past sessions. Fixed by forcing `sys.stdout.reconfigure(encoding="utf-8")` and reordering so `DELETE`+`commit` always runs before any printing. Running the fixed script purged 120 rows that should have been gone long ago (confirmed clean on rerun: 0 removed). Notably, **LIT Charleston's "98 pricing items" (celebrated two paragraphs up) turned out to be entirely hallucinated — all 98 rows purged, listing dropped from published.** Lesson: extraction item *count* proves nothing; only a working validation step does. **Real corrected total: 119 published** (Charleston 16->13, Knoxville 8->7; all other metros unchanged). No metro fell below `MIN_LISTINGS=5` as a result. Verified live post-deploy: Knoxville's 10x10 tent now shows $150-$200, not $0.
...
- Picked **Charleston, SC** (major real wedding-destination market, adjacent to our existing Greenville SC) and **Knoxville, TN** (same mid-size-Southeast tier as our other metros) — both outside tentrentalfinder's 50-city list too. Added to `pipeline/metros.py`.
- First pass (core cities + a Mount Pleasant SC suburb top-up, ~15+ real business sites attempted) came back with **near-total `web_enrich.py` scrape failures** ("no content") and initially got misdiagnosed as a genuine data ceiling — wrong, corrected below.
- **Also found and fixed a real bug along the way**: `geo_validate.py` does exact-string city matching with no fuzzy/alias handling, and Google Maps returns the abbreviated "Mt Pleasant" while we'd seeded "Mount Pleasant" — 0 confirmed matches until caught. Fixed in both the live DB row and `metros.py` (commit `02bf29c`).
- **Root cause of the scrape failures found and fixed**: `FIRECRAWL_API_KEY` in `.env` was invalid (confirmed via a direct `401 "Invalid token"` API test, independent of a separate red-herring bug in my own ad-hoc debugging script that made an earlier "it's fine" check meaningless — see below). Human re-authorized via Firecrawl's own browser OAuth flow (`Path D` in their onboarding doc: generate a PKCE challenge, human authorizes in-browser, poll for the token) to get a fresh, verified-working key.
  - **Process note for future sessions**: my first attempt at diagnosing this used an inline Python script with `load_dotenv(Path('.').parent / '.env')` — a bug, since `Path('.').parent` in pathlib is lexically `'.'` again (no filesystem resolution without `.resolve()`), so it silently loaded no key at all and sent `Authorization: Bearer None`, which produced the exact same 401 error as a genuinely bad key. This produced a false "confirmed same error, key is bad" read at one point. The real pipeline scripts (`web_enrich.py` etc.) always used the correct `Path(__file__).parent.parent / ".env"` and were never affected by that specific bug — but they were still hitting the actual invalid key, which was a real, separate problem. Always use `Path(__file__).resolve().parent` (or import the actual pipeline module, which resolves paths correctly on import) in any one-off diagnostic script — never `Path('.').parent`.
- **Re-ran `web_enrich.py` on all 47 previously-failed candidates with the fixed key: 36 succeeded** (vs. near-total failure before — definitive proof the key was the real blocker, not dead/blocked business sites). `ai_extract` pulled real pricing from 17 of them; `validate_pricing.py` caught and removed 87 hallucinated rows from the larger batch (consistent ~6% rate, same quality gate working as always).
- **Result: 100 → 113 published listings. Three metros crossed `MIN_LISTINGS=5` in one pass:**
  - **Charleston, SC: 4 → 10** (well past the floor — CHEERS, Ruth's House Event Rentals, both Republic Event Rentals locations, plus Loluma, III STAR PARTY RENTAL, CAPITAL PARTY RENTAL, WELLINGTON PARTY BRENTAL, and more, all with real re-scraped pricing)
  - **Knoxville, TN: 0 → 5** (exactly at the floor — Snyder Events, Tailgate Tennessee, Silent Disco Knox, Mechanical Bull Rentals, Ultimate Mega Parties)
  - **Greenville, SC: 4 → 5** — been stuck below the floor since session 18, previously concluded (wrongly, in hindsight) to be a genuine data ceiling. It was hit by the exact same broken-key issue: Greer Event Rentals, which failed earlier this same session, succeeded on the re-run with 37 real pricing items. **Session 20's "accept as-is, genuine data ceiling" conclusion for Greenville should be treated with the same skepticism** — it may well have been this same class of infra issue rather than a real ceiling, though it wasn't re-tested this session and the key's failure history before today is unknown.
  - Also minor bumps: Richmond 5→6 (Richmond Bounce and Party Inflatables, 20 items).
- All three new pages verified live (200, after brief Cloudflare edge-cache propagation lag on the first check). `metros.ts` already had configs for Charleston/Knoxville from the failed first pass, so they activated with zero code change — exactly the auto-graduate design working as intended.
- **Apify spend, fresh account this session: $0 → $4.74 of $5** (human provided a new key mid-session after the first account ran low). **~$0.26 left, effectively exhausted again** — another fresh account needed to continue.
...
- **This-month tier, also finished same session:**
  - **National `/tent-rental-cost/` guide** — the biggest content gap identified (no page targeted the national head terms: "tent rental cost", "wedding tent rental cost", "how much does a tent rental cost", all Easy KD). Built entirely from real cross-metro data: `tentSizeBreakdown()` now takes an optional `minCount` so the national page can drop single-data-point sizes (17 sizes shown, down from 43 raw) while metro pages keep showing everything they have. Also added `deliveryFeeRange()`/`depositRange()` helpers — real published delivery fees ($45-$400) and deposits ($40-$400) across the whole dataset, another real-data answer no competitor has. FAQPage schema included.
  - **Internal linking (S4)**: footer now lists every published metro + the new guide page, site-wide — metro pages previously had no way to link to each other.
  - **Homepage redesign (U1, S5)**: the old per-state sub-grids produced lonely full-width single-card rows for states with only 1-2 metros (FL/IN/OH/VA). Replaced with one unified grid, state as a small corner tag instead of a section heading, and each card now shows a real price teaser ("10x10 tents from $65") computed from the same data the metro page shows.
  - **Mobile nav (B7/U5 remainder)**: the 3-metro subset in the header was wrapping into cramped multi-line rows on narrow viewports. Dropped it below a 640px breakpoint — the footer already has the full metro list on every page, so mobile nav is just brand + About now.
  - **B8 fixed**: Charlotte Party Rentals' only pricing row was `tent_frame: $1,000`, sourced from "Minimum project size: $1,000." — an order minimum mislabeled as a tent price. Re-scraped their real inventory page via Firecrawl, parsed all 32 real "from $X" tent prices deterministically (regex, not an LLM — the source markdown was already clean structured product data). One-off script: `pipeline/fix_charlotte_party_rentals_tents.py`. Re-ran `validate_pricing.py` (0 unsupported) and `export_json.py` (96 published, unchanged) before shipping.
  - **Pipeline→website sync gap closed (roadmap #19)**: `export_json.py` now writes directly to `website/src/data/listings.json`, no more manual `cp` step — this session alone needed that copy 3 separate times. Also renamed the stale `charlotte-metro-listings.json` output to `listings.json` (site has covered 7 metros for many sessions).
- **Still open / deliberately not done this session:**
  - **S6 (monthly re-enrich freshness cadence)** — an ongoing operational cadence, not a one-time fix. All pricing was last checked 2026-07-11/12/14 (the last date reflects the B8 re-scrape); next re-enrich pass should be scheduled, not done ad hoc.
  - **B10 (self-host listing photos)** — still hotlinking `lh3.googleusercontent.com`. Acceptable risk at $0 budget per the original audit; real effort (download 95 images at build/pipeline time, decide storage) for a currently-working dependency. Left as a deliberate later item, not forgotten.
...
17. ~~Get a Web3Forms (or similar) key~~ — DONE 2026-07-14 (Session 21). Lead-capture form live on all 7 metro pages.
18. ~~Set up `www.eventrentalcosts.com` redirect~~ — DONE 2026-07-13. Verified 301, paths preserved.
19. ~~Build a proper sync step from `pipeline/export/*.json` into `website/src/data/`~~ — DONE 2026-07-14 (Session 21). `export_json.py` writes both locations directly now.
20. **Pittsburgh re-checked session 22, confirmed still a genuine ceiling** — unlike Greenville, Pittsburgh's candidates were actually scraped successfully back in session 11 (real content, not blocked by the key bug fixed this session) and the LLM legitimately found no real pricing on any of them. The one remaining never-attempted candidate (Cliq 360 PhotoBooth) was tried with the now-working key and genuinely failed to scrape (dead/blocked site). Stays at 1 listing, correctly unpublished — no further action needed unless new Pittsburgh businesses appear on a future Apify re-ingest. Charleston (10) and Knoxville (5) both crossed the floor session 22. Apify key has ~$0.26/$5 free credit left (fresh account provided session 22) — human will provide another fresh account when it runs out.
21. Schedule the monthly re-enrich freshness cadence (Session 21's Fable-5 analysis, S6) — not yet automated or scheduled, still manual/ad hoc.
22. Self-host listing photos at build time instead of hotlinking Google's CDN (Fable-5 analysis, B10) — deliberately deferred, acceptable risk at $0 budget.
23. **Month-6 kill-switch check: ~2027-01-13.** Check Search Console impressions; invest more / build site 2 if climbing, stop if flat, per the brief's framework.
