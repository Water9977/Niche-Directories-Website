...
20. Consider more metros / suburb top-ups for the metro still below the 5-listing floor: **Pittsburgh** (stuck at 1 since session 11). Given session 22 found that Greenville's session-20 "accept as-is, genuine data ceiling" conclusion was actually a broken-`FIRECRAWL_API_KEY` artifact (fixed session 22, Greenville then immediately crossed to 5), **Pittsburgh's "genuinely quote-gated market" conclusion from session 11 should be re-tested with a working key before being trusted** — it predates today's fix by many sessions and was never re-verified. Charleston (10) and Knoxville (5) both crossed the floor session 22. Apify key has ~$0.26/$5 free credit left (fresh account provided session 22) — human will provide another fresh account when it runs out.
