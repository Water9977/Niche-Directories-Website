...
### 2026-07-16 — Session 22: Charleston SC + Knoxville TN candidate metros, real ceiling hit on both
...
- Ran the full pipeline on both, including a widened-search-terms pass on Charleston (session 8's playbook) since it landed right at the edge:
  - **Charleston: 114 raw → 80 confirmed → 4 published** (CHEERS, Ruth's House Event Rentals, both Republic Event Rentals locations). 1 short of `MIN_LISTINGS=5`. Investigated the remaining candidates directly rather than guessing: several real party-rental businesses exist (Charleston Bounce, Party Boys Rentals, Bell and Bash, Lowcountry Beach Rentals, etc.) but Apify returned no address for them at all — since `export_json.py` requires a real address to publish, these can never cross the gate regardless of pricing. The rest (III STAR PARTY RENTAL, Southern Chic, Loluma, Shrunk 3D, Dryvebox, A Little Pop of Color, LIT Charleston) have real addresses but consistently failed to scrape (retried twice, same "no content" result both times — not a transient Firecrawl hiccup).
  - **Knoxville: 0 published.** The one candidate that looked promising, All Occasions Party Rentals, had `ai_extract` pull 7 "pricing" rows from what turned out to be FAQ boilerplate ("Q. How does the pricing work? A. Most prices quoted are for a one day event...") — `validate_pricing.py` correctly caught and removed every single row since none matched a real dollar figure in context. A real finding, not a pipeline bug: the quality gate did exactly its job.
- **Both metros correctly held below the publish floor** — no page built for either, homepage listing/metro count unchanged (91/7) after deploy, verified live (`/charleston-sc/` returns 404 as expected). Added both to `website/src/lib/metros.ts` anyway (matches the established Pittsburgh/Greenville pattern) so they auto-activate the moment a future re-enrich pass crosses the floor, no code change needed then.
- **Tried a Mount Pleasant SC suburb top-up** since Charleston was 1 short — hit a real bug first: `geo_validate.py` does exact-string city matching with no fuzzy/alias handling, and Google Maps returns the abbreviated "Mt Pleasant" while we'd seeded "Mount Pleasant" — 0 confirmed matches until caught. Fixed the naming in both the live DB row and `metros.py` (commit `02bf29c`), re-validated → 42 confirmed. Enriched all 8 category-matched candidates (2 via the normal queue, 6 via a direct targeted call since the queue's global review-count ordering kept surfacing the same already-failed candidates from other metros) — **all 8 failed to scrape** ("no content"). Confirmed this isn't a Firecrawl-side issue (it scraped Charlotte Party Rentals's real inventory page fine earlier this same session) — genuinely fragile/blocked small-business sites. Between the core-city attempt, the extra-terms pass, and this suburb, that's 15+ real Charleston-area business sites tried with consistent failures — called it a real ceiling for this session rather than continuing to spend credit chasing it.
- **Apify spend this session: $3.06 → $4.74 of the $5 free-tier credit (~$0.26 left, effectively exhausted).** Human will provide a fresh account/key to continue.
- Total published listings: 100 (up from 96). Still 7 live metro pages — Charleston stays at 4 (1 short), Knoxville at 0, Greenville/Pittsburgh unchanged, all correctly unpublished.
...
20. Consider more metros / suburb top-ups for the metros still below the 5-listing floor (Richmond crossed the floor session 18; Greenville/Pittsburgh decided accept-as-is session 20; Charleston SC added session 22, sitting at 4/5 — closest to crossing, worth a suburb top-up (Mount Pleasant/North Charleston already seeded) next time there's fresh Apify credit; Knoxville TN added session 22, 0 published, genuine ceiling this round). Apify key has ~$1.03/$5 free credit left — human will provide a fresh account when it runs out.
